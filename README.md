...
Wireshark running on the Windows VM capturing network traffic.  
...
SSH connection from the Windows VM to the Ubuntu Server.  
...
NSG inbound rules (before and after blocking ICMP).  
...
Ping failing after ICMP is blocked in the NSG.  
...
DNS or HTTP/HTTPS packets captured in Wireshark.  
